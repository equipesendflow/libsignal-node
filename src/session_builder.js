...
class SessionBuilder {
	constructor(storage, signalAddress) {
		this.addr = signalAddress;
		this.storage = storage;
	}

	initOutgoing(device) {
		const fqAddr = this.addr + '.0';

		// if (!this.storage.isTrustedIdentity(this.addr, device.identityKey)) {
		//     throw new errors.UntrustedIdentityKeyError(this.addr, device.identityKey);
		// }
		...
		this.storage.storeSession(fqAddr, record);
	}

	async initIncoming(record, message) {
		// const fqAddr = this.addr + '.0';
		// if (!await this.storage.isTrustedIdentity(fqAddr, message.identityKey)) {
		//     throw new errors.UntrustedIdentityKeyError(this.addr, message.identityKey);
		// }
